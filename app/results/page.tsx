...
export default function ResultsDashboard() {
  // Grab the results object populated from the submit route
  const { results } = useStore();
  
  // Safe fallbacks in case user refreshes page
  const score = results?.score || 0;
  const percentile = results?.percentile || 23; // Will be real in production

  const shareRank = () => {
    const text = `I'm in the top ${percentile}% of Backend Devs. What about you?`;
    if (navigator.share) {
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <Card className="bg-gradient-to-br from-blue-900/20 to-neutral-900 border-blue-500/30">
          <CardContent className="p-8 text-center flex flex-col items-center justify-center">
            ...
            <h2 className="text-6xl font-black text-white mb-2">Top {percentile}%</h2>
            <p className="text-xl text-neutral-300">You scored {score}/100 on the dynamic systems test.</p>
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card className="bg-neutral-900 border-green-500/20">
            ...
            <CardContent className="space-y-4 text-sm text-neutral-300">
              <div className="bg-neutral-950 p-3 rounded-md border border-neutral-800">
                <span className="font-bold text-white">PostgreSQL Indexing:</span> Strong on B-Tree vs Hash index selection.
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-neutral-900 border-red-500/20">
            ...
            <CardContent className="space-y-4 text-sm text-neutral-300">
              <div className="bg-neutral-950 p-3 rounded-md border border-neutral-800">
                <span className="font-bold text-white">WebSocket Scaling:</span> Missed concepts on sticky sessions.
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-neutral-900 border-yellow-500/20">
            ...
            <CardContent className="space-y-2 text-sm text-neutral-300">
              <p>• Master connection pooling strategies (PgBouncer).</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-neutral-900 border-orange-500/20">
            ...
            <CardContent className="space-y-2 text-sm text-neutral-300">
              <p>• <span className="text-white font-medium">Market Shift:</span> Competitors are demonstrating stronger system design fundamentals.</p>
            </CardContent>
          </Card>
        </div>
      </div>
